terraform {
	backend "s3" {
	bucket= "marwa-terraform-state-bucket-e80d8f95" # Replace with your S3 bucket name
	key = "aws-backend/main/terraform.tfstate" # Location of the state file in the bucket
	region = "us-east-1" # AWS region
	dynamodb_table = "marwa-terraform-state-locks" # Replace with your DynamoDB table name
...
resource "aws_s3_object" "private_key_object" {
  bucket = "marwa-terraform-state-bucket-e80d8f95"  # Reference existing S3 bucket
